...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  const { mutate: updateCV, isPending } = useMutateApi<
    CvProcessResponse,
    Error,
  ...
  } = useGetQuery<CvProcessResponse>(`/api/getCv?email=${encodeURIComponent(user?.email || "")}`, {
  ...
  return (
    <aside
      className={`bg-gray-800 text-white w-64 min-h-screen ${isOpen ? "" : "hidden"} md:block`}
    >
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <div className="relative">
              ...
              {user.cv_suggestions && user.cv_suggestions.length > 0 && (
                <div className="absolute top-[-5px] right-0 bg-red-500 w-5 h-5 rounded-full cursor-pointer flex items-center justify-center">
                  <p
                    className="text-white text-xs text-center font-bold"
                    onClick={() => setIsCvTipsDialogOpen(true)}
                  >
                    !
                  </p>
                </div>
      ...
      <Dialog
        open={isCvTipsDialogOpen}
        onOpenChange={setIsCvTipsDialogOpen}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>CV Tips</DialogTitle>
            <DialogDescription>Here is what to improve in your CV:</DialogDescription>
          </DialogHeader>
          {user.cv_suggestions?.map((tip, index) => <p key={index}>~{tip}</p>)}
        </DialogContent>
      </Dialog>
    </aside>
  );
