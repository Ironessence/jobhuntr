...
const JobCard: React.FC<JobCardProps> = ({
  ...
  const shortenDescription = (description: string, maxLength: number = 250) => {
  ...
  return (
    <>
      <div
        className="shadow-md rounded-md p-4 mb-4 cursor-pointer hover:shadow-lg  border border-gray-500 min-w-[300px] w-full md:w-[40%] min-h-[200px] hover:bg-gray-900 duration-200 transition-all ease-in-out"
        onClick={handleClick}
      >
        <h3 className="font-bold text-lg mb-1">{jobTitle}</h3>
        <p className="text-gray-300 text-sm mb-2">{company}</p>
        <p className="text-gray-400 text-sm">{shortenDescription(jobDescription)}</p>
      </div>
      ...
      >
        <DialogContent
          className="max-w-4xl w-11/12 h-3/4 flex flex-col"
        ...
        >
          ...
          <div className="flex-grow overflow-hidden mb-4">
            <div className="h-full overflow-y-auto pr-4">
              <p
                id="job-description"
                className="text-gray-300 text-base whitespace-pre-wrap"
              >
                {jobDescription}
              </p>
            </div>
